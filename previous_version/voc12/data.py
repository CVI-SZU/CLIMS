...
sys.path.append("/data/home/wittlu/wittlu/code/SEAM-master/")
import numpy as np
import torch
...
class VOC12ClipImageDataset(Dataset):

    def __init__(self, img_name_list_path, voc12_root, transform=None):
        self.img_name_list = load_img_name_list(img_name_list_path)
        self.voc12_root = voc12_root
        self.transform = transform
        self.normalize_fn_1 = Normalize()
        self.normalize_fn_2 = Normalize2()
    ...
    def __getitem__(self, idx):
        ...
        if self.transform:
            img = self.transform(img)
            img_1 = self.normalize_fn_1(img)
            img_2 = self.normalize_fn_2(img)
            # print(img_1.mean(), img_2.mean())
...
class VOC12ClsClipDataset(VOC12ClipImageDataset):
    ...
    def __getitem__(self, idx):
        name, img_1, img_2 = super().__getitem__(idx)

        label = torch.from_numpy(self.label_list[idx])

        return name, img_1, img_2, label

class VOC12OKSImageDataset(Dataset):

    def __init__(self, img_name_list_path, voc12_root, transform=None):
        self.img_name_list = load_img_name_list(img_name_list_path)
        self.sub_name_list = self.img_name_list.copy()
        self.voc12_root = voc12_root
        self.transform = transform
    ...
    def __getitem__(self, idx, sub=False):
        if sub:
            name = self.sub_name_list[idx]
        else:
            name = self.img_name_list[idx]
...
class VOC12OKSDataset(VOC12OKSImageDataset):

    def __init__(self, img_name_list_path, voc12_root, transform=None):
        super().__init__(img_name_list_path, voc12_root, transform)
        self.label_list = load_image_label_list_from_npy(self.img_name_list)
        self.sub_label_list = self.label_list.copy()
    ...
    def __getitem__(self, idx):
        ...
        # del(self.sub_label_list[idx])
        # del(self.sub_name_list[idx])
        valid = (self.label_list[idx] == self.sub_label_list)
        valid = ((valid == 0).sum(axis=-1) == 0)
        valid[idx] = 0
        indices = list(np.where(valid == 1)[0])

        if len(indices) != 0:
            select = np.random.randint(0, len(indices))

            sub_name, sub_img = super().__getitem__(indices[select], sub=True)
            sub_label = torch.from_numpy(self.sub_label_list[indices[select]])

            return img, label, sub_img, sub_label
        else:
            return img, label
