...
parser.add_argument('--gt_dir', default='/home/lzr/data/VOC/VOC2012/SegmentationClass', type=str)
...
if ip == '172.31.234.159':
    if args.domain == 'train_aug':
        args.gt_dir = '/data1/xjheng/dataset/VOC2012/SegmentationClassAug'
    else:
        args.gt_dir = '/data1/xjheng/dataset/VOC2012/SegmentationClass'
elif ip == '172.31.111.180':
    if args.domain == 'train_aug':
        args.gt_dir = '/home/lzr/data/VOC/VOC2012/SegmentationClassAug'
    else:
        args.gt_dir = '/home/lzr/data/VOC/VOC2012/SegmentationClass'
else:
...
def compare(start,step,TP,P,T, name_list):
    for idx in range(start,len(name_list),step):
        ...
        if os.path.isfile(predict_folder + name + '.npy'):
            # copy cam
            # import os
            # path = os.path.join(predict_folder, name + '.npy')
            # os.system(f'cp {path} /data1/xjheng/workspace/programs/7th-WSeg-CLIP/for_sota159_1_train/')
...
if __name__ == '__main__':
    ...
    import csv
    headers = ['Exp_Name', 'mIOU','Threshold']
    if not os.path.exists('ablation_studies.csv'):
        with open('ablation_studies.csv', 'w') as f:
            f_csv = csv.writer(f)
    ...
    if args.mode == 'png':
        ...
        if args.threshold is None:
            ...
            rows = [args.experiment_name, best_mIoU, best_th]
            with open('ablation_studies.csv', 'a') as f:
                f_csv = csv.writer(f)
                f_csv.writerow(rows)
        else:
            loglist = do_python_eval(args.predict_dir, args.gt_dir, name_list, 21)
