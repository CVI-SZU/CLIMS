...
class _BaseDatasetCotrain(data.Dataset):
    """
    Base dataset class
    """
    ...
    def _augmentation(self, image, label, label2):
        # Scaling
        h, w = label.shape
        if self.base_size:
            if h > w:
        ...
        label2 = Image.fromarray(label2).resize((w, h), resample=Image.NEAREST)
        ...
        if pad_h > 0 or pad_w > 0:
            image = cv2.copyMakeBorder(image, value=self.mean_bgr, **pad_kwargs)
            label = cv2.copyMakeBorder(label, value=self.ignore_label, **pad_kwargs)
            label2 = cv2.copyMakeBorder(label2, value=self.ignore_label, **pad_kwargs)
        ...
        if self.flip:
            ...
            if random.random() < 0.5:
                image = np.fliplr(image).copy()  # HWC
                label = np.fliplr(label).copy()  # HW
                label2 = np.fliplr(label2).copy()  # HW
        return image, label, label2

    def __getitem__(self, index):
        image_id, image, label, label2 = self._load_data(index)
        if self.augment:
            image, label, label2 = self._augmentation(image, label, label2)
        # Mean subtraction
        image -= self.mean_bgr
        # HWC -> CHW
        image = image.transpose(2, 0, 1)
        return image_id, image.astype(np.float32), label.astype(np.int64), label2.astype(np.int64)
...
class _BaseDatasetTest(data.Dataset):
    """
    Base dataset class
    """
